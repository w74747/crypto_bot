...
class ScalpingOrchestrator:
    ...
    async def _process_inner(
        ...
        if fg_signal == "strong_buy":
            # خوف شديد + BTC تشبع بيعي = فرصة نادرة جداً
        ...
        # إذا BTC نفسه في تشبع بيعي شديد (<25) → السماح بغض النظر عن F&G
        if btc_rsi <= 25:
            _log(f"[BTC ⚡] {symbol}: BTC RSI={btc_rsi:.1f} تشبع بيعي شديد — السماح بالصفقة")
