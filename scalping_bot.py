...
class ScalpingOrchestrator:
    ...
    async def _process_inner(
        ...
        if fg_signal == "strong_buy":
            effective_btc_min = max(25, self.cfg.s1_btc_rsi_min - 15)  # أكثر تساهلاً
            _log(f"[F&G ✅] {symbol}: خوف شديد {fg_value}/100 — BTC حد مخفف إلى {effective_btc_min}")
        elif fg_signal == "buy":
            effective_btc_min = max(30, self.cfg.s1_btc_rsi_min - 5)
            _log(f"[F&G ✅] {symbol}: خوف {fg_value}/100 — BTC حد مخفف إلى {effective_btc_min}")
        else:
            _log(f"[F&G] {symbol}: {fg_value}/100 ({fg_label}) — حد BTC عادي {effective_btc_min}")

        if btc_rsi < effective_btc_min:
            _log(f"[BTC Filter ❌] {symbol}: BTC RSI={btc_rsi:.1f} < {effective_btc_min}")
            return
        _log(f"[BTC Filter ✅] {symbol}: BTC RSI={btc_rsi:.1f} | F&G={fg_value}/100")
    ...
    async def _send_smart_report(self):
        ...
        data_summary = f"""
حالة البوت الآن:
- الرصيد الحر (Spot): ${spot_balance:.2f}
...
"""
        ...
        prompt = f"""أنت محلل بوت تداول عملات رقمية. راجع هذه البيانات واكتب تقريراً مختصراً بالعربية (5-8 أسطر فقط).

{data_summary}

التقرير يجب أن يتضمن:
1. الحالة العامة (✅ جيد / ⚠️ تحتاج انتباه / 🔴 مشكلة)
2. أبرز الصفقات المفتوحة وحالتها
3. أي مشاكل أو مخاوف تقنية واضحة
4. توصية قصيرة واحدة إن وجدت

اكتب بأسلوب مباشر ومختصر. لا تذكر معلومات غير ضرورية."""
                ...
                async with session.post(
                    ...
                    if resp.status == 200:
                        ...
                        msg = (
                            ...
                            f"{report}\n\n"
                            "━━━━━━━━━━━━━━━━━━━━\n"
                            f"💼 رصيد Spot: <code>${spot_balance:.2f}</code> | "
