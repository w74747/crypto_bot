...
class TradeMonitor:
    ...
    async def _check_slot(self, state: SlotState):
        ...
        if state.sl_order_id:
            sl_status = await asyncio.get_event_loop().run_in_executor(
        ...
        if state.tp1_order_id:
            tp1_status = await asyncio.get_event_loop().run_in_executor(
        ...
        if state.tp3_order_id:
            tp3_status = await asyncio.get_event_loop().run_in_executor(
...
class ScalpingOrchestrator:
    ...
    async def _process_candidate(
        ...
        stop_loss = await asyncio.get_event_loop().run_in_executor(
