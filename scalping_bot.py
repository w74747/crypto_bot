...
class Config:
    ...
    # رأس المال — يُحدَّث ديناميكياً من scan_loop كل دورة
    # TRADE_INVESTMENT_AMOUNT = 0 → يعني ديناميكي تلقائياً
    # TRADE_INVESTMENT_AMOUNT = 10 → يعني ثابت $10
    _capital_override: float = field(default_factory=lambda: float(os.environ.get("TRADE_INVESTMENT_AMOUNT", "0")))
    capital:           float = field(default_factory=lambda: float(os.environ.get("TRADE_INVESTMENT_AMOUNT", "10")))
    ...
    monitor_interval:   int   = 10  # كل 10 ثواني — مراقبة لحظية
    ...
    s3_enabled:         bool  = field(default_factory=lambda: os.environ.get("S3_BREAKOUT_ENABLED", "true").lower() == "true")
    s3_breakout_pct:    float = field(default_factory=lambda: float(os.environ.get("S3_BREAKOUT_PCT", "2.0")))     # ارتفاع % في 4H
    s3_volume_ratio:    float = field(default_factory=lambda: float(os.environ.get("S3_VOLUME_RATIO", "1.8")))     # حجم × المتوسط
    s3_tp1_pct:         float = field(default_factory=lambda: float(os.environ.get("S3_TP1_PCT", "2.0")))          # هدف +2%
    s3_sl_pct:          float = field(default_factory=lambda: float(os.environ.get("S3_SL_PCT", "1.0")))           # وقف -1%
    s3_rsi_max:         float = field(default_factory=lambda: float(os.environ.get("S3_RSI_MAX", "70")))           # لا ندخل إذا RSI > 70 (تشبع شرائي)

    # ── Breakout Momentum (S3) ──
    s3_enabled:         bool  = field(default_factory=lambda: os.environ.get("S3_BREAKOUT_ENABLED", "true").lower() == "true")
    s3_candle_pct:      float = field(default_factory=lambda: float(os.environ.get("S3_CANDLE_PCT", "2.0")))      # ارتفاع الشمعة %
    s3_vol_ratio:       float = field(default_factory=lambda: float(os.environ.get("S3_VOL_RATIO", "1.5")))       # نسبة الحجم
    s3_tp1_pct:         float = field(default_factory=lambda: float(os.environ.get("S3_TP1_PCT", "2.0")))         # هدف %
    s3_sl_pct:          float = field(default_factory=lambda: float(os.environ.get("S3_SL_PCT", "1.0")))          # وقف خسارة %
...
class TradeMonitor:
    ...
    async def start(self):
        ...
        self._failed_tasks   = []   # مهام فاشلة تنتظر إعادة المحاولة
        _log("[TradeMonitor] ✅ started — مراقبة كل 15 ثانية")
        while self._running:
            # ── مراقبة كل الصفقات ──
            try:
                await self._check_all_slots()
            except Exception as e:
                _log(f"[TradeMonitor] ⚠️ خطأ في _check_all_slots: {e} — متابعة")

            # ── إعادة المحاولة للمهام الفاشلة ──
            if self._failed_tasks:
                await self._retry_failed_tasks()

            # ── Reconcile كل reconcile_interval ──
            ...
            if self._reconcile_tick >= self.cfg.reconcile_interval:
                ...
                try:
                    ...
                    _log(f"[TradeMonitor] ⚠️ خطأ في reconcile: {e} — متابعة")

            await asyncio.sleep(self.cfg.monitor_interval)

    async def _retry_failed_tasks(self):
        """إعادة محاولة المهام الفاشلة — لا تُهمل أي مهمة."""
        remaining = []
        for task in self._failed_tasks:
            task["attempts"] = task.get("attempts", 0) + 1
            # emergency_sell لا يتوقف أبداً حتى ينجح
            max_attempts = 999 if task.get("action") == "emergency_sell" else 10
            if task["attempts"] > max_attempts:
                _log(f"[Retry] {task['symbol']}: تجاوز {max_attempts} محاولات — تجاهل")
                continue
            try:
                symbol = task["symbol"]
                action = task["action"]
                state  = self.slots.get_state(symbol)
                if not state:
                    continue
                if action == "re_place_tp":
                    ...
                    if new_ids.get("tp1_order_id"):
                        ...
                        _log(f"[Retry] ✅ {symbol}: {action} نجحت في المحاولة {task['attempts']}")
                        continue
                    ...
                    if ok:
                        ...
                        _log(f"[Retry] ✅ {symbol}: emergency_sell نجح")
                        continue
                remaining.append(task)
            except Exception as e:
                _log(f"[Retry] {task['symbol']} محاولة {task['attempts']}: {e}")
                remaining.append(task)
        self._failed_tasks = remaining
    ...
    async def _audit_slot(self, state: SlotState, open_order_ids: set):
        ...
        if fallback_id:
            ...
            # تسجيل كمهمة فاشلة لإعادة المحاولة
            if hasattr(self, "_failed_tasks"):
                self._failed_tasks.append({"symbol": symbol, "action": "re_place_tp", "attempts": 0})
                _log(f"[Retry Queue] {symbol}: أُضيفت لقائمة الإعادة")
            await self._notify(
...
class ScalpingOrchestrator:
    ...
    async def _process_breakout_candidate(
        self,
        session:         aiohttp.ClientSession,
        symbol:          str,
        initial_balance: float = 0.0,
        ...
        """
        استراتيجية S3 — Breakout Momentum.

        المنطق:
        - العملة ارتفعت > S3_BREAKOUT_PCT% في آخر شمعة 4H
        - الحجم > S3_VOLUME_RATIO × متوسط 20 شمعة
        - RSI < S3_RSI_MAX (لم يتشبع بعد)
        - لا تحتاج BTC Filter أو Fear & Greed

        الأهداف:
        - TP1: +S3_TP1_PCT% (افتراضي +2%)
        - SL: -S3_SL_PCT% (افتراضي -1%)
        - R:R = 1:2 — معقول جداً

        الفلسفة: صفقات سريعة كثيرة بدل انتظار الفرصة المثالية
        """
        if not self.cfg.s3_enabled:
            return
        ...
        try:
            ...
            ohlcv = await asyncio.get_running_loop().run_in_executor(
                None,
                lambda: self.executor.exchange.fetch_ohlcv(symbol, "4h", limit=25)
            )
            if not ohlcv or len(ohlcv) < 5:
                return
            ...
            rsi_series  = 100 - 100 / (1 + df["close"].diff().clip(lower=0).rolling(14).mean() /
                          (-df["close"].diff().clip(upper=0).rolling(14).mean().replace(0, 1e-9)))
            curr_rsi    = float(rsi_series.iloc[-1])

            # ── فلتر Breakout ──
            breakout_pct = (curr_close - prev_close) / prev_close * 100
            vol_ratio    = curr_vol / avg_vol if avg_vol > 0 else 0

            if breakout_pct < self.cfg.s3_breakout_pct:
                return  # لم يرتفع بما يكفي
            if vol_ratio < self.cfg.s3_volume_ratio:
                return  # الحجم غير مقنع
            if curr_rsi > self.cfg.s3_rsi_max:
                return  # تشبع شرائي — متأخر جداً

            _log(
                f"[S3 Breakout] {symbol}: +{breakout_pct:.1f}% "
                f"Vol×{vol_ratio:.1f} RSI={curr_rsi:.0f}"
            )

            # ── فلتر حجم التداول (بسيط) ──
            vol_usd = curr_vol * curr_close
            if vol_usd < 50_000:  # $50K حد أدنى
                return

            # ── Balance Guard ──
            capital = self.cfg.capital if self.cfg.capital > 0 else (initial_balance / self.cfg.max_slots)
            try:
                bal = self.executor.exchange.fetch_balance({"type": "spot"})
                free_usdt = float(bal.get("USDT", {}).get("free", 0) or bal.get("free", {}).get("USDT", 0))
                if free_usdt < capital * 0.9:
                    return
            except Exception:
                return

            # ── الأهداف (بسيطة ومباشرة) ──
            tp1       = curr_close * (1 + self.cfg.s3_tp1_pct / 100)
            tp2       = curr_close * (1 + self.cfg.s3_tp1_pct / 100 * 1.5)
            tp3       = curr_close * (1 + self.cfg.s3_tp1_pct / 100 * 2)
            stop_loss = curr_close * (1 - self.cfg.s3_sl_pct / 100)
            ...
            state = await asyncio.get_running_loop().run_in_executor(
                None, self.executor.execute_full_trade,
                symbol, curr_close, tp1, tp2, tp3, stop_loss,
            )
            ...
            # تسجيل في DB
            trade_id = await asyncio.get_running_loop().run_in_executor(
                None, lambda: self.db.insert_trade(
                    state=state, capital=capital,
                    ds_vote="AUTO", llama_vote="AUTO",
                    rss_sentiment="breakout",
                    galaxy_score=0,
                    committee_summary=f"S3 Breakout +{breakout_pct:.1f}% Vol×{vol_ratio:.1f} RSI={curr_rsi:.0f}",
                )
            )
            if trade_id:
                self.slots.update_state(symbol, db_trade_id=trade_id)
            ...
            await self._send_telegram(
                "⚡ <b>صفقة Breakout — S3</b>\n"
                "━━━━━━━━━━━━━━━━━━━━\n\n"
                f"📌 <b>العملة:</b> <code>{symbol}</code>\n"
                f"📈 <b>Breakout:</b> <code>+{breakout_pct:.1f}%</code> | "
                f"Vol×<code>{vol_ratio:.1f}</code> | RSI=<code>{curr_rsi:.0f}</code>\n"
                f"💰 <b>رأس المال:</b> <code>${capital:.2f}</code>\n\n"
                "━━━━━━━━━━━━━━━━━━━━\n"
                f"🎯 TP1: <code>{tp1:.6g}</code> (+{self.cfg.s3_tp1_pct:.1f}%)\n"
                f"🎯 TP2: <code>{tp2:.6g}</code> (+{self.cfg.s3_tp1_pct*1.5:.1f}%)\n"
                f"🛡 SL: <code>{stop_loss:.6g}</code> (-{self.cfg.s3_sl_pct:.1f}%)"
            )
...
"""
    ...
    async def _auto_heal_positions(self):
        ...
        try:
            ...
            for sym, row in db_trades.items():
                ...
                dust_threshold = getattr(self.cfg, "capital", self.cfg._capital_override or 10) * 0.20
            ...
            for asset, qty in balances.items():
                ...
                try:
                    ...
                    value = qty * price
                    dust_limit = getattr(self.cfg, "capital", self.cfg._capital_override or 10) * 0.20
                    if value < dust_limit:
                        # أتربة — تجاهل
    ...
    async def _process_breakout_candidate(
        ...
        الفكرة: عملة ارتفعت > S3_CANDLE_PCT% في شمعة 4H مع حجم مرتفع
        = زخم حقيقي → ندخل ونأخذ +2% سريعاً

        لا تحتاج BTC Filter — تعمل في أي اتجاه للسوق.
        """
        if not self.cfg.s3_enabled:
            return
        ...
        try:
            # ── جلب OHLCV 4H ──
            ind = await asyncio.get_running_loop().run_in_executor(
                None, self._fetch_breakout_indicators, symbol
            )
            if not ind:
                return

            # ── فلتر الحجم ──
            if ind["vol_ratio"] < self.cfg.s3_vol_ratio:
                return

            # ── فلتر الشمعة ──
            if ind["candle_pct"] < self.cfg.s3_candle_pct:
                return

            # ── EMA Trend ──
            if ind["ema9"] <= ind["ema21"] * 0.99:  # تسامح 1%
                return

            _log(
                f"[S3 Breakout] {symbol}: +{ind['candle_pct']:.1f}% | "
                f"Vol×{ind['vol_ratio']:.1f} | EMA✅"
            )

            # ── CMC Filter (خفيف) ──
            async with aiohttp.ClientSession() as s:
                cmc = await self.pipeline.get_cmc_data(s, symbol)
            if cmc.get("rank", 9999) > 500 and cmc.get("rank", 0) < 999:
                return  # خارج Top 500 وليس بدون CMC

            # ── Balance Check ──
            capital = getattr(self, "_dynamic_capital", initial_balance or 10.0)
            try:
                bal = self.executor.exchange.fetch_balance({"type": "spot"})
            ...
            entry = ind["current"]
            tp1   = entry * (1 + self.cfg.s3_tp1_pct / 100)
            tp2   = entry * (1 + self.cfg.s3_tp1_pct / 100 * 1.5)
            tp3   = entry * (1 + self.cfg.s3_tp1_pct / 100 * 2.0)
            sl    = entry * (1 - self.cfg.s3_sl_pct / 100)

            # ── تنفيذ ──
            state = await asyncio.get_running_loop().run_in_executor(
                None, self.executor.execute_full_trade,
                symbol, entry, tp1, tp2, tp3, sl,
            )
            if not state:
                return
            ...
            trade_id = await asyncio.get_running_loop().run_in_executor(
                ...
                lambda: self.db.insert_trade(
                    state=state, capital=capital,
                    ds_vote="S3", llama_vote="BUY",
                    rss_sentiment="breakout",
                    galaxy_score=0,
                    committee_summary=(
                        f"S3 Breakout +{ind['candle_pct']:.1f}% "
            ...
            await self._send_telegram(
                "⚡ <b>صفقة Breakout جديدة</b>\n"
                "━━━━━━━━━━━━━━━━━━━━\n\n"
                f"📌 <b>العملة:</b> <code>{symbol}</code>\n"
                f"📈 <b>الشمعة:</b> +{ind['candle_pct']:.1f}% | حجم×{ind['vol_ratio']:.1f}\n"
                f"💰 <b>رأس المال:</b> <code>${capital:.2f}</code>\n"
                f"📈 <b>سعر الدخول:</b> <code>{entry:.8g}</code>\n\n"
                "━━━━━━━━━━━━━━━━━━━━\n"
                f"🎯 TP1: <code>{tp1:.8g}</code> (+{self.cfg.s3_tp1_pct:.1f}%)\n"
                f"🛡 SL:  <code>{sl:.8g}</code> (-{self.cfg.s3_sl_pct:.1f}%)"
            )
    ...
    def _fetch_breakout_indicators(self, symbol: str) -> dict | None:
        """مؤشرات Breakout Momentum — شمعة 4H + حجم + EMA."""
        try:
            ohlcv = self.executor.exchange.fetch_ohlcv(symbol, "4h", limit=30)
            ...
            # آخر شمعة مكتملة (قبل الأخيرة)
            last  = df.iloc[-2]
            candle_pct = (last["close"] - last["open"]) / last["open"] * 100
            if candle_pct <= 0:  # شمعة هابطة — لا نهتم
                return None

            # نسبة الحجم
            avg_vol   = float(df["vol"].iloc[-21:-1].mean())
            vol_ratio = float(last["vol"]) / avg_vol if avg_vol > 0 else 0
            ...
            try:
                t = self.executor.exchange.fetch_ticker(symbol)
                vol_usd = float(t.get("quoteVolume") or 0)
                current = float(t.get("last") or closes.iloc[-1])
            except Exception:
                vol_usd = 0.0
                current = float(closes.iloc[-1])
    ...
    async def scan_loop(self):
        ...
        await self._send_telegram(
            ...
            f"🚀 S3 (Breakout): {'✅' if self.cfg.s3_enabled else '⏸'} شمعة +{self.cfg.s3_candle_pct:.0f}% | Vol×{self.cfg.s3_vol_ratio:.1f} | TP +{self.cfg.s3_tp1_pct:.0f}% | SL -{self.cfg.s3_sl_pct:.0f}%\n"
        ...
        while True:
            ...
            try:
                ...
                # تحديث cfg.capital ديناميكياً في كل دورة
                object.__setattr__(self.cfg, "capital", dynamic_capital)
                ...
                _log(
                    ...
                    f"{'ديناميكي' if self.cfg._capital_override == 0 else 'ثابت'}"
                )
                ...
                try:
                    ...
                    async with aiohttp.ClientSession() as session:
                        for i in range(0, len(all_symbols), BATCH):
                            ...
                            tasks = []
                            dyn_cap = getattr(self, "_dynamic_capital",
                                initial_balance / max(1, self.cfg.max_slots))
                            for market_type, sym in batch:
                                if market_type == "spot":
                                    tasks.append(self._process_candidate(session, sym, dyn_cap))
                                    tasks.append(self._process_momentum_candidate(session, sym, dyn_cap))
                                    if self.cfg.s3_enabled:
                                        tasks.append(self._process_breakout_candidate(session, sym, dyn_cap))
                                else:
                                    tasks.append(self._process_futures_candidate(session, sym, dyn_cap))
