...
async def specialist_fib(session, opp: TradeOpportunity) -> tuple[str, dict]:
    ...
    # الحد الأقصى 15% للـ Scalping — مع ضمان tp1 < tp2 < tp3
    entry = opp.entry_price
    cap   = entry * 1.15

    tp1 = max(tp1, entry * 1.03)           # لا يقل عن 3% فوق الدخول
    tp2 = max(tp2, tp1   * 1.03)           # لا يقل عن 3% فوق tp1
    tp3 = max(tp3, tp2   * 1.03)           # لا يقل عن 3% فوق tp2

    # caps متدرجة تمنع TP2=TP3 في النطاقات الضيقة
