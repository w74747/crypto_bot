...
يتكامل مع Portfolio Manager لتحديد المبلغ تلقائياً
"""
...
class TradeExecutor:
    ...
    def __init__(self):
        ...
        self.portfolio = None   # يُعيَّن بعد الإنشاء
    ...
    def init_portfolio(self):
        """يهيّئ Portfolio Manager بعد الاتصال"""
    ...
    def _get_trade_amount(self, symbol: str) -> float:
        """يحدد مبلغ الصفقة من Portfolio أو الإعداد الثابت"""
        if PORTFOLIO_MODE and self.portfolio:
            amount = self.portfolio.calculate_trade_amount()
            return amount
    ...
    def place_market_buy(
        ...
        logger.info(
            ...
            f"سعر: ~{entry_price:.8g} | مبلغ: ${amount_usd:.2f}"
        )
        try:
            self._ensure_markets_loaded()

            qty   = round(amount_usd / entry_price, 4)
    ...
    def place_tp_and_sl_orders(
        ...
        logger.info(
            f"[Executor] أوامر {symbol} (أهداف التوصية):\n"
            f"  TP1={tp1:.8g} x{qty1} | "
            f"TP2={tp2:.8g} x{qty2} | "
            f"TP3={tp3:.8g} x{qty3} | "
            f"SL={stop_loss:.8g}"
