...
البوت لا يتوقف إذا فشل الاتصال
"""
...
def _get_conn():
    """يُعيد اتصال بـ PostgreSQL مع دعم Session Pooler"""
    import psycopg2
    url = DATABASE_URL
...
def init_db():
    ...
    لا يُوقف البوت عند فشل الاتصال
    """
    if not DATABASE_URL:
        logger.warning("[DB] DATABASE_URL غير موجود — البوت يعمل بدون قاعدة بيانات")
        return
...
def log_signal(opp, debate: dict, btc_price: float = 0.0,
               btc_trend: str = "safe", galaxy_score: float = 0.0) -> str:
    if not DATABASE_URL:
        return ""
...
def log_user_decision(signal_id: str, decision: str,
                      opp=None, filled_price: float = 0.0,
...
def update_trade_outcome(signal_id: str, exchange):
    ...
    try:
        ...
        cur.execute("""
        SELECT t.symbol, t.filled_price, m.tp1, m.tp2, m.tp3, m.stop_loss
        ...
        cur.close()
        conn.close()
        logger.info(f"[DB Tracker] {symbol} | {pct_change:+.1f}% | {status}")
