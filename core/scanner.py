...
class MarketScanner:
    ...
    def passes_filters(self, symbol: str, ind: dict, volume_usd: float) -> tuple[bool, str]:
        ...
        # ── RSI Filter — من Railway env ──
        rsi_threshold = int(os.environ.get("RSI_OVERSOLD_THRESHOLD", "31"))
        if rsi > rsi_threshold:
            logger.debug(f"[RSI] {symbol}: RSI={rsi:.1f} > {rsi_threshold}")
            return False, ""
        ...
        # استراتيجية B
        if distance <= MAX_DISTANCE_FROM_LOD and rsi < rsi_threshold:
