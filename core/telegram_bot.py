"""
core/telegram_bot.py — مع عرض الأهداف الديناميكية
"""
...
def format_with_debate(opp: TradeOpportunity, debate: dict) -> str:
    ...
    risk_model = debate.get("risk_model", "Risk")

    c_sum  = _trim(finals.get("Claude",   "—"))
    d_sum  = _trim(finals.get(risk_model, finals.get("DeepSeek", finals.get("Groq", "—"))))
    gk_sum = _trim(finals.get("Grok",     "—"))
    r_sum  = _trim(reddit, 90)
