...
async def cmd_buy(update, context):
    ...
    try:
        import ccxt
        import pandas as pd
        from core.scanner import TradeOpportunity, calculate_dynamic_targets, calculate_rsi
        import numpy as np

        # استخدم اتصال الـ executor الموجود — بدون إنشاء scanner جديد
        if _executor is None:
            await update.message.reply_text("❌ البوت لم يكتمل تهيئته، انتظر دقيقة")
            return
        ...
        nearest_support = float(lows.tail(14).min())
        local_high      = float(highs.tail(60).max())
        crash_60d       = (local_high - current_price) / local_high if local_high > 0 else 0
        lod_180         = float(lows.tail(180).min())
        distance        = (current_price - lod_180) / lod_180 if lod_180 > 0 else 1.0

        # أهداف بسيطة بـ Fibonacci
        fib_range = local_high - nearest_support
        tp1 = nearest_support + fib_range * 0.236
        tp2 = nearest_support + fib_range * 0.382
        tp3 = nearest_support + fib_range * 0.618
        tp1 = max(tp1, current_price * 1.05)
        tp2 = max(tp2, tp1 * 1.08)
        tp3 = max(tp3, tp2 * 1.10)
        ...
        opp = TradeOpportunity(
            ...
            crash_pct_60d     = crash_60d,
            lod_180           = lod_180,
            distance_from_lod = distance,
            rsi_daily         = rsi,
            ...
            tp2               = round(tp2, 10),
            tp3               = round(tp3, 10),
            tp_method         = "Daily Fib (يدوي)",
            fib_high          = local_high,
            fib_low           = nearest_support,
        )
