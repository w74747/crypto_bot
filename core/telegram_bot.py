...
async def cmd_buy(update, context):
    """
    /buy XRP
    ينفّذ التوصية الموجودة في الذاكرة بأرقامها الأصلية
    إذا لم توجد توصية → يُخبر المستخدم
    """
    if not context.args:
        await update.message.reply_text(
            "⚠️ استخدم الأمر هكذا:\n<code>/buy XRP</code>",
    ...
    # البحث عن التوصية في الذاكرة
    opp = _pending.get(clean)

    if not opp:
        await update.message.reply_text(
            f"⚠️ لا توجد توصية حالية لـ <code>{symbol}</code>\n\n"
            f"انتظر حتى يكتشف البوت فرصة في دورة الفحص القادمة،\n"
            f"أو استخدم /status لرؤية الفرص المتاحة.",
            parse_mode="HTML"
        )
        return

    # تأكيد الأرقام للمستخدم
    await update.message.reply_text(
        f"⏳ جاري تنفيذ توصية {symbol}\n"
        f"💰 المبلغ: ${TRADE_AMOUNT_USD}\n"
        f"🎯 الدخول: <code>{opp.entry_price:.8g}</code>\n"
    ...
    try:
        ...
        if result.get("success"):
            signal_id = _signal_ids.get(clean, "")
            if signal_id:
                from core.database import log_user_decision
...
async def cmd_status(update, context):
    ...
    try:
        if _executor and _executor.portfolio:
            portfolio_text = _executor.portfolio.get_status_text()
            text += f"\n\n{portfolio_text}"
    except Exception:
        pass
    ...
    try:
        ...
        if stats_text and "لا توجد" not in stats_text:
            text += f"\n\n📊 <b>Trading Brain</b>\n{stats_text}"
    except Exception:
        pass
    ...
    async def send_and_execute(
        ...
        """ينفذ الصفقة تلقائياً ويرسل رسالة تأكيد"""
        from config.settings import TRADE_AMOUNT_USD, PORTFOLIO_MODE
        ...
        if result.get("success"):
            ...
            logger.info(f"[Auto Execute] ✅ {opp.symbol} | ID: {sent.message_id}")
            return sent.message_id

        else:
            # فشل — أرسل للمستخدم مع زر إعادة المحاولة
            error = result.get("error", "خطأ غير معروف")
            text  = (
                f"⚠️ <b>فشل التنفيذ التلقائي</b>\n\n"
                f"العملة: <code>{opp.symbol}</code>\n"
                f"السبب: {error}\n\n"
                f"اضغط 🔄 لإعادة المحاولة يدوياً"
            )
            clean = opp.symbol.replace("/", "_")
            _pending[clean]    = opp
            _signal_ids[clean] = signal_id
            sent = await self.bot.send_message(
                chat_id=TELEGRAM_CHAT_ID, text=text,
                parse_mode="HTML",
                reply_markup=create_retry_keyboard(opp.symbol),
            )
            return sent.message_id
    ...
    async def send_info_only(
        ...
        """يرسل الفرصة كمعلومات فقط — بدون أزرار تنفيذ"""
        base = format_opportunity(opp)

        info_text = (
            f"📋 <b>فرصة #{rank} — للمعلومية</b>\n\n"
            + base
        )
        ...
        sent = await self.bot.send_message(
            ...
            text       = info_text,
            parse_mode = "HTML",
            # بدون reply_markup — لا أزرار
        )
        logger.info(f"[Info Only] 📋 {opp.symbol} #{rank} | ID: {sent.message_id}")
        return sent.message_id
